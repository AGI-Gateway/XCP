...
# ── revocation, signed ─────────────────────────────────────────────────────
# Found in audit (XCP-A-02): the gossip endpoint took the originating peer's
# domain from the request body. Since the peer list was also readable, anyone
# could name a real peer and revoke any node. A claim of identity is not
# evidence of it — the originator must sign.

def _revocation_typed(payload: dict[str, Any], chain_id: int) -> dict:
    return {
        "types": {
            "EIP712Domain": [{"name": "name", "type": "string"},
                             {"name": "version", "type": "string"},
        ...
        "domain": {"name": "XCPRevocation", "version": "1", "chainId": chain_id},
        "message": payload,
    }


def sign_revocation(identity: NodeIdentity, subject_node_id: str,
                    reason: str = "", now: Optional[int] = None,
                    chain_id: int = 8453) -> dict[str, Any]:
    """Assert, with proof, that this node revokes another."""
    if not _ETH:
        raise IdentityError("eth-account required to sign a revocation")
    if not identity.private_key:
        raise IdentityError("signing needs the node's private key")
    payload = {"subject": subject_node_id, "issuer": identity.node_id,
               "issuedAt": now or int(time.time()), "reason": reason[:200]}
    sig = Account.sign_message(
        encode_typed_data(full_message=_revocation_typed(payload, chain_id)),
        private_key=identity.private_key).signature.hex()
    return {**payload, "signature": sig if sig.startswith("0x") else "0x" + sig}


def verify_revocation(doc: dict[str, Any], expected_issuer_node_id: str,
                      max_age: int = 3600, now: Optional[int] = None,
                      chain_id: int = 8453) -> list[str]:
    """
    Check a revocation actually came from the node it names. Returns problems.
    """
    problems: list[str] = []
    n = now or int(time.time())
    payload = {k: doc.get(k) for k in ("subject", "issuer", "issuedAt", "reason")}
    if not payload["subject"] or not payload["issuer"]:
        return ["revocation is missing subject or issuer"]
    if str(payload["issuer"]).lower() != expected_issuer_node_id.lower():
        problems.append("revocation issuer is not the peer it claims to be")
    age = n - int(payload.get("issuedAt") or 0)
    if age > max_age or age < -300:
        problems.append("revocation is stale or dated in the future; replaying "
                        "an old revocation must not take a live node down")
    sig = doc.get("signature", "")
    if not sig:
        problems.append("revocation is unsigned — a claim is not evidence")
    elif _ETH:
        try:
            rec = Account.recover_message(
                encode_typed_data(
                    full_message=_revocation_typed(payload, chain_id)),
                signature=sig)
            if node_id_from_address(rec) != payload["issuer"]:
                problems.append("signature does not match the declared issuer")
        except Exception as e:
            problems.append(f"revocation signature failed to recover: {e}")
    return problems


__all__ = [
    "sign_revocation", "verify_revocation",
