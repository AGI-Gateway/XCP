...
test: ## run every test suite
	...
	@echo "── telemetry ──"          && $(PY) tests/test_telemetry.py
	@echo "── config drift ──"       && $(PY) tests/test_config_drift.py
	...
	@echo "── security audit ──"     && $(PY) tests/test_security_audit.py
	...
	@echo "── oauth bridge ──"       && $(PY) tests/test_oauth_bridge.py
	@echo "── delegation ──"         && $(PY) tests/test_delegation.py
	@echo "── privacy ──"            && $(PY) tests/test_privacy.py
	@echo "── covenants ──"          && $(PY) tests/test_covenants.py
...
analysis: ## adversarial analysis: fuzzing + cross-implementation differential
	$(PY) security/analysis/fuzz_binding.py
	$(PY) security/analysis/differential.py
...
.PHONY: help install demo test lint certs up image contracts bench analysis publish docs docs-sync docs-serve clean
