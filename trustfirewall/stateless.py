...
class NonCanonicalHeader(ValueError):
    """Raised when a routing header is not in canonical form."""


def _canonical_field(value: str, field: str) -> str:
    """
    Reject a routing field that is not already canonical, rather than
    normalising it.

    FOUND BY ADVERSARIAL ANALYSIS (draft Appendix B.3, item 1). This function
    previously called .strip(), which made "search" and "search " produce the
    SAME binding digest. That is a parser differential in its most dangerous
    shape: the gateway strips, an upstream may not, so a credential verified
    for one tool name can be forwarded carrying a different one. The gateway
    would see a valid binding for the call it checked while the origin executed
    the call it was sent.

    Normalising here would close this instance and leave the class open, since
    it assumes every downstream parser normalises identically. Refusing
    non-canonical input keeps the two parsers from being able to disagree.
    """
    v = value or ""
    if v != v.strip():
        raise NonCanonicalHeader(
            f"{field} has leading or trailing whitespace; routing headers must "
            "be sent in canonical form so that every parser in the chain reads "
            "the same value")
    if any(ord(c) < 0x20 or ord(c) == 0x7F for c in v):
        raise NonCanonicalHeader(f"{field} contains a control character")
    return v
...
def bind_digest(mcp_method: str, mcp_name: str, body: bytes = b"") -> str:
    ...
    Binds the values as sent. It does NOT normalise them -- see
    _canonical_field for why normalising was the bug.
    """
    return digest(canonical({
        "m": _canonical_field(mcp_method, "Mcp-Method"),
        "n": _canonical_field(mcp_name, "Mcp-Name"),
        "b": digest(body or b""),
    }))
