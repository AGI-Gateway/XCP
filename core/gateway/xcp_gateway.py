...
# Administrative authentication. Found in audit: /admin/revoke accepted a
# footprint from anyone who could reach the port and revoked that session —
# a one-request denial of service against every agent on the node. The endpoint
# is now disabled unless a token is configured, because an admin surface that
# defaults to open is worse than one that does not exist.
_ADMIN_TOKEN = os.getenv("XCP_ADMIN_TOKEN", "")
if not _ADMIN_TOKEN:
    print("[gateway] /admin/* is DISABLED: set XCP_ADMIN_TOKEN to enable it.",
          flush=True)


def _admin_ok(request: "Request") -> bool:
    """Constant-time check. A short-circuiting compare leaks the token."""
    import hmac
    if not _ADMIN_TOKEN:
        return False
    hdr = request.headers.get("authorization", "")
    presented = hdr[7:] if hdr.lower().startswith("bearer ") else ""
    return hmac.compare_digest(presented, _ADMIN_TOKEN)


# ── policy ─────────────────────────────────────────────────────────────────
# The policy engine and its reversibility classes existed as a tested library
# with no reference in the gateway — the same defect as the federation module
# before it, and the same consequence: a control nobody could actually reach.
# XCP_POLICY points at a policy document; XCP_POLICY_APPROVED_DIGEST is what an
# auditor approved, so the node can report whether the policy it is ENFORCING is
# the policy that was signed off.
_POLICY = None
_POLICY_ATTEST: dict = {}
_policy_path = os.getenv("XCP_POLICY", "")
if _policy_path:
    try:
        import sys as _ps, pathlib as _pp
        _ps.path.insert(0, str(_pp.Path(__file__).resolve().parents[2]))
        from policy import parse_text, evaluate as _policy_eval, Request as _PReq
        _POLICY = parse_text(_pp.Path(_policy_path).read_text())
        problems = _POLICY.validate()
        if problems:
            raise RuntimeError("; ".join(problems))
        approved = os.getenv("XCP_POLICY_APPROVED_DIGEST", "")
        _POLICY_ATTEST = _POLICY.to_dict()
        if approved:
            _POLICY_ATTEST["approvedDigest"] = approved
            same = _POLICY_ATTEST.get("digest") == approved
            _POLICY_ATTEST["matchesApproved"] = same
            if not same:
                print("[gateway] WARNING: the enforced policy does NOT match the "
                      "approved digest. Treat decisions since the change as "
                      "unapproved.", flush=True)
    except Exception as _e:
        # A policy that cannot load is a control the operator believes they have.
        raise RuntimeError(
            f"XCP_POLICY is set but could not be loaded ({_e}). Refusing to "
            "start: running without a policy you asked for is worse than "
            "failing to start.")


def _policy_check(ctx: "Ctx", action: str, args: dict):
    """Evaluate institutional policy. Returns a rejection response or None."""
    if _POLICY is None:
        return None
    try:
        amt = args.get("amount_minor") or args.get("amountMinor")
        req = _PReq(subject=str(ctx.agent_id or "anonymous"), action=action,
                    tier=getattr(ctx.binding, "tier", "") or "",
                    amount_minor=int(amt) if amt is not None else None,
                    currency=str(args.get("currency", "")),
                    target_domain=str(args.get("domain", "")))
        d = _policy_eval(_POLICY, req)
    except Exception as e:
        # Fail closed: an evaluation error is not a permit.
        return JSONResponse({"error": f"XCP policy evaluation failed: {e}"},
                            status_code=403)
    if getattr(d, "allowed", False):
        return None
    record_decision(None, decision="block", reason=getattr(d, "reason", "policy"),
                    stream="policy", agent_id=ctx.agent_id, scope=action)
    audit(ctx, "policy", f"DENIED {action}: {getattr(d, 'reason', '')}")
    return JSONResponse({"error": "XCP policy denied this action",
                         "reason": getattr(d, "reason", ""),
                         "policyDigest": _POLICY_ATTEST.get("digest", "")},
                        status_code=403)


# ── conformance profile ────────────────────────────────────────────────────
# Independent review (GPT-5.6, findings B.4.3, B.4.4, B.4.7) observed that every
# safe choice in this gateway was optional: self-registration was available with
# no verifier, the argument firewall was opt-in, and the session path coexisted
# with request credentials. Each was individually documented and collectively
# they meant a deployment could satisfy none of them and still call itself XCP.
#
# The profile makes them mandatory together. A production node refuses to start
# unless it can actually enforce what the profile claims -- which is the only
# form of a security property that survives contact with a hurried deployment.
_PROFILE = os.getenv("XCP_PROFILE", "development").strip().lower()
if _PROFILE not in ("development", "production"):
    raise RuntimeError(f"XCP_PROFILE must be development or production, "
                       f"not {_PROFILE!r}")

if _PROFILE == "production":
    _missing = []
    if POSTURE != "enforce":
        _missing.append("XCP_POSTURE must be 'enforce'")
    if not os.getenv("XCP_VERIFY_URL", ""):
        _missing.append(
            "XCP_VERIFY_URL must be set: without an external verifier a caller "
            "can create its own identity binding, so the trust layer becomes a "
            "self-attestation mechanism")
    if os.getenv("XCP_SECURITY", "0") != "1":
        _missing.append("XCP_SECURITY must be 1")
    if os.getenv("XCP_RATE_LIMIT", "1") == "0":
        _missing.append("XCP_RATE_LIMIT must not be 0")
    if os.getenv("XCP_LIMIT_FAIL_OPEN", "0") == "1":
        _missing.append("XCP_LIMIT_FAIL_OPEN must not be 1")
    if not os.getenv("XCP_ADMIN_TOKEN", ""):
        _missing.append("XCP_ADMIN_TOKEN must be set")
    if os.getenv("XCP_ALLOW_INSECURE_PEERS", "0") == "1":
        _missing.append("XCP_ALLOW_INSECURE_PEERS must not be 1")
    if _missing:
        raise RuntimeError(
            "XCP_PROFILE=production but the deployment cannot enforce it:\n  - "
            + "\n  - ".join(_missing)
            + "\nRefusing to start. A profile that is claimed and not enforced "
              "is worse than one that was never claimed.")
else:
    print("[gateway] XCP_PROFILE=development. Self-registration is permitted "
          "and hardening is optional. NOT for production.", flush=True)

PRODUCTION = _PROFILE == "production"

# FOUND BY INDEPENDENT REVIEW (B.4.6). XCP-Upstream-Credential is relayed
# without inspection, which is right for confidentiality and incomplete as a
# contract: an unbounded, unspecified pass-through header is a confused-deputy
# surface. The gateway still never reads, logs or stores the value; it now
# bounds what it will carry.
MAX_UPSTREAM_CREDENTIAL = int(os.getenv("XCP_MAX_UPSTREAM_CREDENTIAL", "8192"))


def _check_upstream_credential(request: "Request", server: str):
    """
    Returns a rejection or None. Enforces size and destination binding without
    inspecting the credential itself.
    """
    raw = request.headers.get("xcp-upstream-credential", "")
    if not raw:
        return None
    if len(raw) > MAX_UPSTREAM_CREDENTIAL:
        return JSONResponse(
            {"error": f"XCP-Upstream-Credential exceeds "
                      f"{MAX_UPSTREAM_CREDENTIAL} bytes"}, status_code=413)
    # Destination binding: a credential offered for one upstream must not be
    # relayed to another, or the gateway becomes a deputy that forwards
    # someone's key wherever the caller points it.
    intended = request.headers.get("xcp-upstream-audience", "")
    if intended and intended != server:
        return JSONResponse(
            {"error": "XCP-Upstream-Credential is bound to a different "
                      "upstream than the one addressed"}, status_code=403)
    if PRODUCTION and not intended:
        return JSONResponse(
            {"error": "XCP-Upstream-Audience is required in the production "
                      "profile so a forwarded credential is bound to one "
                      "destination"}, status_code=400)
    return None
...
def _fed_limit(request: "Request", what: str):
    """
    Meter federation traffic on its own budget. Sharing the anonymous bucket
    starved legitimate peering, because that bucket is sized for hostile
    callers rather than for peers who cannot authenticate yet.
    """
    if _LIMITS is None:
        return None
    key = "fed:" + (getattr(getattr(request, "client", None), "host", "") or "anon")
    d = _LIMITS.check(agent_key=key, tier="A1xH0", method="federation",
                      scope=what)
    if not d.allowed:
        record_limit("federation")
...
def check_mandate(mandate_hdr: str, binding: Binding | None,
    ...
    # FOUND BY DIFFERENTIAL ANALYSIS against the Node implementation, which
    # refused both of the following while this one forwarded them upstream.
    #
    # A mandate with no notAfter was treated as never expiring, because
    # `int(proof.get("notAfter", 0)) and ...` short-circuits to falsy when the
    # field is absent and the expiry check was therefore skipped entirely. An
    # absent expiry is not an infinite one; a delegation that cannot lapse is
    # not a delegation.
    try:
        not_after = int(proof.get("notAfter") or 0)
    except (TypeError, ValueError):
        return False, "mandate notAfter is not an integer"
    if not_after <= 0:
        return False, "mandate has no expiry; an absent expiry is not an " \
                      "infinite one"
    if not_after < int(time.time()):
        return False, "mandate expired"

    # A mandate whose scope list is absent, null, or not a list was reaching
    # the coverage check and passing it vacuously. Malformed authorisation is
    # absent authorisation, not universal authorisation.
    scopes = proof.get("mandateScope")
    if not isinstance(scopes, list):
        return False, "mandateScope must be a list of scopes"
    if not scopes:
        return False, "mandateScope is empty; an empty mandate permits nothing"
...
def parse_ctx(request: Request) -> Ctx:
    ...
    # Strict rather than coercive. int() accepts "+42001", " 42001" and
    # "42_001", all of which differ from what another parser in the chain
    # reads from the same bytes. Found by differential analysis against the
    # Node implementation, which refused what this one silently normalised.
    if not (parts[0].isdigit() and parts[1].isdigit()):
        return Ctx(reason="identity fields must be canonical decimal integers")
...
async def session_open(request: Request) -> JSONResponse:
    ...
    In the development profile with no external verifier this accepts a
    self-asserted identity, which is why the production profile refuses to
    start without one.
    """
    if _PROFILE == "production" and not VERIFY_URL:
        return JSONResponse(
            {"error": "self-registration is not available in the production "
    ...
    if not VERIFY_URL:
        if PRODUCTION:
            # Reachable only if XCP_VERIFY_URL was unset, which the profile
            # check forbids -- belt and braces, because this is the path that
            # turns the trust layer into a self-attestation mechanism.
            return JSONResponse(
                {"error": "self-registration is not available in the production "
                          "profile; obtain a binding from the configured "
...
async def session_close(request: Request) -> JSONResponse:
    limited = _limit(Ctx(), "tools/list", request=request)
...
async def a2t_call(request: Request) -> Response:
    ...
    bad_cred = _check_upstream_credential(request, server)
    if bad_cred is not None:
        return bad_cred

    denied = _policy_check(ctx, scope, body.get("arguments") or {})
...
async def a2a_delegate(request: Request) -> Response:
    ...
    # FOUND BY INDEPENDENT REVIEW (B.4.5). This previously returned
    # accepted=True, which a caller could reasonably read as delivery. The
    # initiating agent's authority to delegate says nothing about whether the
    # destination authenticated or executed anything.
    return JSONResponse({"authorized": True, "delivered": False, "peer": peer,
...
async def admin_revoke(request: Request) -> JSONResponse:
    ...
    if not _admin_ok(request):
        _fed_audit("admin/revoke refused (no valid admin token)")
        return JSONResponse(
            {"error": "administrative endpoints require a bearer token; set "
                      "XCP_ADMIN_TOKEN on the gateway"}, status_code=401)
    limited = _limit(Ctx(), "tools/call", request=request)
    if limited is not None:
        return limited
    body = await request.json()
...
async def federation_peer(request: Request) -> JSONResponse:
    ...
    limited = _fed_limit(request, "peer")
    if limited is not None:
        return limited
    ...
    if not domain or not url:
        return JSONResponse({"error": "domain and gatewayUrl are required"},
                            status_code=400)
    # The URL comes from an unauthenticated caller, so it is attacker-controlled
    # until proven otherwise. Validate the RESOLVED address, not the string.
    try:
        from security.xcpsec.egress import check_url, EgressDenied, MAX_FETCH_BYTES
        check_url(url)
    except EgressDenied as e:
        _fed_audit(f"peer fetch refused for {domain}: {e}")
        return JSONResponse({"error": f"peer URL refused: {e}"}, status_code=400)
    except Exception:
        return JSONResponse({"error": "peer URL refused"}, status_code=400)

    try:
        from federation import (NodeRecord, protocol_incompatible,
                                verify_node_record, digest as _fp_digest)
        import httpx
        # TLS verification stays ON. It was disabled here, which meant a peer
        # record fetched over https could be substituted by anyone on the path
        # — and peering is precisely where trust is established.
        ...
        if r.status_code >= 300:
            # Redirects are refused rather than followed: an open redirector on
            # a benign host would otherwise defeat the egress check above.
            return JSONResponse({"error": "peer record must be served directly"},
                                status_code=502)
        if len(r.content) > MAX_FETCH_BYTES:
            return JSONResponse({"error": "peer record is too large"},
                                status_code=502)
        doc = r.json()
        rec = NodeRecord(**{k: v for k, v in doc.items()
                            if k in NodeRecord.__dataclass_fields__})
    except Exception:
        # Generic on purpose: echoing the fetch error turned a blind SSRF into
        # an oracle that reported what the gateway could see.
        return JSONResponse({"error": "could not retrieve a valid peer record"},
                            status_code=502)
    ...
    # FOUND BY INDEPENDENT REVIEW (GPT-5.6, finding B.1). The library has
    # verify_node_record(), which binds the record to the certificate the peer
    # actually presented -- and this endpoint never called it. The record's
    # self-declared domain was being taken as sufficient, so an unauthenticated
    # caller controlling both `domain` and `gatewayUrl` could have any record
    # promoted to VERIFIED.
    #
    # The certificate is now retrieved from the live endpoint and the record is
    # verified against it. Over plain HTTP there is no certificate to bind to,
    # so the peer CANNOT reach VERIFIED: it is recorded at the observed level
    ...
    if url.startswith("https://"):
        try:
            ...
            with socket.create_connection((u.hostname, u.port or 443), timeout=8) as sock:
                with ctx_ssl.wrap_socket(sock, server_hostname=u.hostname) as ss:
                    der = ss.getpeercert(binary_form=True)
            problems = verify_node_record(rec, der, domain,
                                          known_sequence=getattr(
                                              _FED.peers.get(domain), "sequence", 0))
            if problems:
                _fed_audit(f"peer {domain} REFUSED: {problems[0]}")
                return JSONResponse({"error": "peer record did not verify",
                                     "reason": problems[0]}, status_code=403)
            tls_verified = True
        except Exception as e:
            _fed_audit(f"peer {domain} certificate check failed: {type(e).__name__}")
            return JSONResponse(
                {"error": "could not bind the peer record to its certificate"},
                status_code=502)
    elif os.getenv("XCP_ALLOW_INSECURE_PEERS", "0") != "1":
        return JSONResponse(
            {"error": "peering over plain HTTP is refused; the record cannot be "
                      "bound to a certificate"}, status_code=400)
    known = _FED.peers.get(domain)
    try:
        # FOUND BY INDEPENDENT REVIEW (GPT-5.6, finding B.2). `mutual` was
        # taken from the request body and passed straight through, so the
        # initiating caller could award itself PEERED -- the STRONGEST trust
        # level -- by setting a boolean. Mutual trust means "we verified them
        # and they verified us"; the second half cannot be self-asserted.
        #
        # This path now grants at most VERIFIED. PEERED requires the peer to
        # confirm independently, which is a separate exchange.
        p = _FED.add_verified_peer(domain, rec.node_id, rec.gateway_url,
                                   mutual=False, sequence=rec.sequence)
    except Exception as e:
    ...
    return JSONResponse({
        ...
        "mutual": False,
        "note": "this path grants at most VERIFIED; PEERED requires the peer "
...
async def federation_peers(request: Request) -> JSONResponse:
    """
    Full topology requires an admin token. Who a node peers with, and at what
    hop count, is exactly the reconnaissance that made forged revocations
    practical — and the privacy design keeps counterparties out of everything
    else it publishes, so publishing them here was inconsistent too.
    """
    if _FED is None:
        return JSONResponse({"error": "federation not configured"}, status_code=404)
    summary = _FED.summary()
    if not _admin_ok(request):
        return JSONResponse({"self": _FED.self_domain, "summary": summary,
                             "peers": "redacted; requires an admin token"})
...
async def federation_revoke(request: Request) -> JSONResponse:
    ...
    if _FED is None:
        return JSONResponse({"error": "federation not configured"}, status_code=404)
    limited = _fed_limit(request, "revoke")
    if limited is not None:
        return limited
    body = json.loads(await request.body() or b"{}")
    frm = str(body.get("from", "")).lower()
    doc = body.get("revocation") or {}
    node_id = str(doc.get("subject") or body.get("nodeId", ""))
    if not node_id or not frm:
        return JSONResponse({"error": "from and a signed revocation are required"},
                            status_code=400)

    # The originating domain used to be taken from the body and trusted. Since
    # the peer list was also public, anyone could name a real peer and revoke
    # any node. The revocation must now be signed by the peer's node key.
    peer = _FED.peers.get(frm)
    if peer is None:
        return JSONResponse({"accepted": False,
                             "reason": "unknown peer"}, status_code=403)
    from federation import verify_revocation
    problems = verify_revocation(doc, peer.node_id)
    if problems:
        _fed_audit(f"revocation REJECTED from {frm}: {problems[0]}")
        return JSONResponse({"accepted": False, "reason": problems[0]},
                            status_code=403)
...
async def health() -> dict:
    ...
    return {"ok": True, "posture": POSTURE, "gateway": GATEWAY_ID, **_adv,
            ...
            "policy": _POLICY_ATTEST or "none configured",
