...
def test_a2a_and_t2t_scopes():
    ...
    a2a = sign_mandate(key, "a", ident.address, [f"a2a:delegate/{peer}"],
                       int(time.time()) + 3600)
    # Reports AUTHORISED, not DELIVERED. The initiating agent's authority to
    # delegate says nothing about whether the destination authenticated or
    # executed anything, and conflating the two was an independent-review
    # finding.
    _d = c.delegate(peer, {"j": 1}, mandate=a2a)
    assert _d["authorized"] is True and _d["delivered"] is False
